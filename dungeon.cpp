...
int telemap_empty(dungeon_t *d){
    uint8_t x, y;
      d->teleport = 0;
  smooth_hardness(d);
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      telemapxy(x, y) = ter_wall;
      if (y == 0 || y == DUNGEON_Y - 1 ||
          x == 0 || x == DUNGEON_X - 1) {
        telemapxy(x, y) = ter_wall_immutable;
        hardnessxy(x, y) = 255;
      }
      charxy(x, y) = NULL;
    }
...
void fog_map(dungeon_t *d){

  int pcX = d->pc.position[dim_x];
  int pcY = d->pc.position[dim_y];
  int minX;
  int minY;
  int maxX = d->pc.position[dim_x] + 2;
  int maxY = d->pc.position[dim_y] + 2;

  for(minX = pcX - 2; minX <= maxX; minX++){
    for(minY = pcY - 2; minY <= maxY; minY++){
      fogxy(minX, minY) = mapxy(minX, minY);

    }
  }
}

void telemap_map(dungeon_t *d){
  //pair_t dest;

  for (int y = 0; y < 21; y++) {
    for (int x = 0; x < 80; x++) {
      telemapxy(x, y) = mapxy(x, y);
    }
  }
