...
uint32_t move_cursor(dungeon_t *d, uint32_t dir)
{
  ...
  case 9:
    ...
    break;
  case '<':
    if (teleport(d->pc.position) == ter_stairs_up) {
      was_stairs = 1;
      new_dungeon_level(d, '<');
    }
    break;
  case '>':
    if (teleport(d->pc.position) == ter_stairs_down) {
      was_stairs = 1;
      new_dungeon_level(d, '>');
    }
    break;
  ...
  if ((dir != '>') && (dir != '<') && (teleport(next) >= ter_floor)) {
    ...
    return 0;
  } else if (teleport(next) < ter_floor) {
    io_queue_message(wallmsg[rand() % (sizeof (wallmsg) /
                                       sizeof (wallmsg[0]))]);
    //io_display_fog(d);
  }
